...
// Конструкторы
Contact::Contact() : FirstName("NoFirstName"), MiddleName("NoMiddleName"), LastName("NoLastName"), BirthDate("00.00.0000"),
Email("noname@no.com"), PhoneNumber("00000000000") {}


Contact::Contact(const std::string& firstName, const std::string& middleName, const std::string& lastName,
...
void Contact::PrintContact() const {
    std::cout << "Contact: " << FirstName << " " << MiddleName << " "
        << LastName << ", Date of birth: " << BirthDate
        << ", Phone number: " << PhoneNumber << ", Email: "
        << Email << std::endl;
}
bool Contact::IsValidName(const std::string& Name) const {
    std::regex NameRegex(R"(^[A-Za-zА-Яа-яЁё][A-Za-zА-Яа-яЁё\- ]*[A-Za-zА-Яа-яЁё]$)", std::regex_constants::ECMAScript);;
