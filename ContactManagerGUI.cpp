...
void ContactManagerGUI::SetupUI() {
    ...
    databaseSaveButton = new QPushButton("Сохранить в БД", this);
    databaseLoadButton = new QPushButton("Загрузить из БД", this);
    ...
    layout->addWidget(databaseSaveButton);
    layout->addWidget(databaseLoadButton);
    ...
    connect(databaseSaveButton, &QPushButton::clicked, this, &ContactManagerGUI::SaveContactsToDatabase);
    connect(databaseLoadButton, &QPushButton::clicked, this, &ContactManagerGUI::LoadContactsFromDatabase);
}
...
void ContactManagerGUI::LoadContactsFromDatabase() {
    contactManager.GetContacts().clear();
    QSqlQuery query("SELECT id, firstName, middleName, lastName, birthDate, email FROM contacts");

    while (query.next()) {
        int id = query.value(0).toInt();
        std::string firstName = query.value(1).toString().toStdString();
        std::string middleName = query.value(2).toString().toStdString();
        std::string lastName = query.value(3).toString().toStdString();
        ...
        phoneQuery.prepare("SELECT type, number FROM phones WHERE contact_id = ?");
        phoneQuery.addBindValue(id);
        phoneQuery.exec();

        while (phoneQuery.next()) {
            phoneNumbers.emplace_back(
                phoneQuery.value(0).toString().toStdString(),
                phoneQuery.value(1).toString().toStdString()
                );
        }

        contactManager.AddContact(Contact(firstName, middleName, lastName, birthDate, email, phoneNumbers));
    }

    LoadContactsToTable();
    QMessageBox::information(this, "Успех", "Контакты загружены из базы данных!");
}
void ContactManagerGUI::SaveContactsToDatabase() {
    QSqlQuery query;
    query.exec("DELETE FROM contacts");
    query.exec("DELETE FROM phones");

    for (const auto& contact : contactManager.GetContacts()) {
        query.prepare("INSERT INTO contacts (firstName, middleName, lastName, birthDate, email) "
                      "VALUES (?, ?, ?, ?, ?)");
        ...
        if (!query.exec()) {
            qWarning() << "Ошибка сохранения контакта:" << query.lastError().text();
            continue;
        }

        int contact_id = query.lastInsertId().toInt();
        for (const auto& phone : contact.GetPhoneNumbers()) {
            QSqlQuery phoneQuery;
            phoneQuery.prepare("INSERT INTO phones (contact_id, type, number) VALUES (?, ?, ?)");
            phoneQuery.addBindValue(contact_id);
            phoneQuery.addBindValue(QString::fromStdString(phone.first));
            phoneQuery.addBindValue(QString::fromStdString(phone.second));
            phoneQuery.exec();
        }
    }
    QMessageBox::information(this, "Успех", "Контакты сохранены в базу данных!");
}
