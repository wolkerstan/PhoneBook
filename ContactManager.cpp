...
bool ContactManager::OpenDatabase(const QString& dbName) {
    ...
    query.exec("CREATE TABLE IF NOT EXISTS phone_numbers ("
               "id INTEGER PRIMARY KEY AUTOINCREMENT, "
               "contact_id INTEGER, "
               "type TEXT, "
               "number TEXT, "
...
ContactManager::ContactManager() {
    OpenDatabase("contacts.db");
    InitDatabase();
}
ContactManager::~ContactManager() {
    CloseDatabase();
}
void ContactManager::InitDatabase() {
    ...
    query.exec("CREATE TABLE IF NOT EXISTS phones ("
               "id INTEGER PRIMARY KEY AUTOINCREMENT, "
               "contact_id INTEGER, type TEXT, number TEXT, "
               "FOREIGN KEY(contact_id) REFERENCES contacts(id))");
}

void ContactManager::CloseDatabase() {
    if (db.isOpen()) {
        db.close();
    }
}
